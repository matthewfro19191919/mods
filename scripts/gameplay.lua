...
function onCreatePost()
    makeLuaSprite('healthbaralt','healthBar2',getProperty('healthBarBG.x'),getProperty('healthBarBG.y'))
    ...
    addHaxeLibrary('Main');
    runHaxeCode([[
        Main.fpsVar.defaultTextFormat = new openfl.text.TextFormat("comic.ttf", 16, -1);
    ]]);
    if downscroll then
    ...
    if songName == 'Polygonized' or songName == 'Interdimensional' and middlescroll then
        setPropertyFromClass('ClientPrefs', 'middleScroll', false) --Just because opponent notes on middle scroll are fucked
    end
    if songName == 'cuzsie-x-kapi-shipping-cute' then
